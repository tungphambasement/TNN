...
int main(int argc, char *argv[]) {
  ...
  Endpoint coordinator_endpoint = Endpoint::ucx(host, port);
  ...
  Endpoint local_worker_endpoint = Endpoint::ucx(local_worker_host, local_worker_port);
  ...
  Vec<Endpoint> endpoints = {
      Endpoint::ucx(worker1_host, worker1_port),
  };

  bool enable_ucx_local_endpoint = false;
  Env::get("UCX_ENABLE_LOCAL_ENDPOINT", enable_ucx_local_endpoint);

  std::cout << "[UCXCFG] remote_worker host=" << worker1_host
            << " port=" << worker1_port
            << " device=" << getenv_str("ROCE_WORKER1_DEVICE", cfg.device_name)
            << " gid=" << getenv_int("ROCE_WORKER1_GID_INDEX", cfg.gid_index)
            << std::endl;

  std::cout << "[UCXCFG] local_endpoint_enabled="
            << (enable_ucx_local_endpoint ? "yes" : "no")
