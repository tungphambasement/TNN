...
#include "tensor/gpu_tensor_ops.hpp"
...
void im2col(const Tensor<T, NCHW> &input_tensor, T *col_data, size_t kernel_h, size_t kernel_w,
  ...
  cuda::im2col(input_data, col_data, batch_size, channels, height, width, kernel_h, kernel_w,
               stride_h, stride_w, pad_h, pad_w, output_h, output_w);
  cuda::synchronize();
}

template <typename T>
void col2im_padded(const T *col_data_raw, T *result_data, size_t batch_size, size_t channels,
  ...
  cuda::col2im(col_data_raw, result_data, batch_size, channels, height, width, kernel_h, kernel_w,
               stride_h, stride_w, pad_h, pad_w, output_h, output_w);
  cuda::synchronize();
}
...
Tensor<T, NCHW> pad(const Tensor<T, NCHW> &input, size_t pad_h, size_t pad_w, T value) {
  ...
  cuda::pad(input_data, result_data, batch_size, channels, height, width, pad_h, pad_w, value);
...
Tensor<T, NCHW> unpad(const Tensor<T, NCHW> &input, size_t pad_h, size_t pad_w) {
  ...
  cuda::unpad(input_data, result_data, batch_size, channels, height, width, pad_h, pad_w);
  cuda::synchronize();
...
Tensor<T, NCHW> crop(const Tensor<T, NCHW> &input, const size_t start_h, const size_t start_w,
  ...
  cuda::crop(input_data, result_data, batch_size, channels, height, width, start_h, start_w,
...
Tensor<T, NCHW> slice_channels(const Tensor<T, NCHW> &input, size_t start_ch, size_t end_ch) {
  ...
  for (size_t n = 0; n < batch_size; ++n) {
    ...
    cuda::cuda_copy(&input_data[src_offset], &result_data[dst_offset], copy_size);
  }
...
template <typename T> void apply_softmax(Tensor<T, NCHW> &input) {
  ...
  cuda::softmax(data, batch_size, num_classes, height, width);
  cuda::synchronize();
}
