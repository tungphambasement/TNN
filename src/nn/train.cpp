...
Result train_epoch(Sequential<T> &model, BaseDataLoader<T> &train_loader, Optimizer<T> &optimizer,
                   Loss<T> &loss_function, const TrainingConfig &config) {
...
template <typename T>
Result validate_model(Sequential<T> &model, BaseDataLoader<T> &test_loader,
  ...
  thread_wrapper.execute([&]() -> void {
    for (int epoch = 0; epoch < config.epochs; ++epoch) {
      ...
      auto [avg_train_loss, avg_train_accuracy] =
          train_epoch(model, train_loader, *optimizer, *loss_function, config);
      auto train_end = std::chrono::high_resolution_clock::now();
      auto train_epoch_duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(train_end - train_start);

      // validation phrase
      auto [avg_val_loss, avg_val_accuracy] = validate_model(model, test_loader, *loss_function);
...
template Result validate_model<float>(Sequential<float> &model, BaseDataLoader<float> &test_loader,
