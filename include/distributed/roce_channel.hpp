...
// ROCE_DIRECT_CREDIT_PATCH
// Experimental large-message fast path with slot credit/ack.
// Sender may write only into remote slots currently marked free.
// Receiver returns slot after deserialization callback completes.
constexpr size_t ROCE_DIRECT_SLOT_COUNT = 4;
constexpr size_t ROCE_DIRECT_SLOT_SIZE = 32ULL * 1024ULL * 1024ULL;
constexpr size_t ROCE_DIRECT_MIN_SIZE = 1ULL * 1024ULL * 1024ULL;
...
public:
  ...
  void enqueue_send(uint64_t msg_serial_id, dptr *data_buffer) {
    ...
    if (direct_slots_ready_ &&
        data_buffer->capacity() >= ROCE_DIRECT_MIN_SIZE &&
        data_buffer->capacity() <= ROCE_DIRECT_SLOT_SIZE) {

      uint32_t slot = UINT32_MAX;
      {
        std::lock_guard<std::mutex> lock(mutex);
        if (!remote_free_slots_.empty()) {
          slot = remote_free_slots_.front();
          remote_free_slots_.pop_front();
        }
      }
      ...
      if (slot != UINT32_MAX) {
        ...
        if (ibv_post_send(qp, &wr, &bad_wr)) {
          ...
          {
            std::lock_guard<std::mutex> lock(mutex);
            remote_free_slots_.push_back(slot);
            inflight_count--;
      ...
      // No free direct slot right now: fall back to safe rendezvous path.
    }
  ...
  void handle_recv_wc(ibv_wc *wc, dptr *buf, const std::function<void(dptr *)> &on_payload_ready) {
    if (wc->opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
      ...
      } else if (header.type == PacketType::SLOT_FREE) {
        uint32_t slot;
        reader(slot);
        process_slot_free(slot);
      }
    }
  ...
  std::deque<uint32_t> remote_free_slots_;
...
private:
  ...
  void process_slot_free(uint32_t slot) {
    if (slot >= ROCE_DIRECT_SLOT_COUNT) return;
    std::lock_guard<std::mutex> lock(mutex);
    remote_free_slots_.push_back(slot);
    inflight_cv.notify_one();
  }

  void send_slot_free(uint32_t slot) {
    auto *send_buf = new dptr(ibv_allocator_.allocate(sizeof(PacketHeader) + sizeof(uint32_t)));
    PacketHeader header(PacketType::SLOT_FREE, sizeof(uint32_t), 0, 0, 1);
    header.msg_serial_id = 0;
    ...
    struct ibv_sge sge;
    sge.addr = (uint64_t)send_buf->get();
    sge.length = sizeof(PacketHeader) + sizeof(uint32_t);
    sge.lkey = ibv_allocator_.get_mr_info(*send_buf).lkey;
    ...
    if (ibv_post_send(qp, &wr, &bad_wr)) {
      std::cerr << "Failed to send SLOT_FREE\n";
      delete send_buf;
    }
  }
