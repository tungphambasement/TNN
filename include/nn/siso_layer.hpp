...
public:
  ...
  Vec<Tensor> forward_impl(const Vec<ConstTensor> &inputs, size_t pid) override {
    ...
    return {forward_impl(inputs[0], pid)};
  }
...
protected:
  virtual Tensor forward_impl(const ConstTensor &input, size_t pid = 0) = 0;
  virtual Tensor backward_impl(const ConstTensor &grad_output, size_t pid = 0) = 0;
