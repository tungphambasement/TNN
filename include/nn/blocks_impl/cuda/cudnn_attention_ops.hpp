...
namespace tnn {
namespace cuda {
namespace cudnn_attn {

// Computes Softmax along the last dimension of a 2D-view of the tensor.
// Input is assumed to be contiguous.
// We treat the input as [rows, cols] and compute softmax over cols.
// For Attention scores [batch*heads, 1, L, L], rows = batch*heads*L, cols = L.
template <typename T>
