...
void MaxPool2DLayer<T>::forward_impl(const Tensor<T> &input, Tensor<T> &output,
  ...
  // Calculate output shape directly from input (implicit padding)
  ...
  forward_task_ = compute_max_pool_forward(input.data_ptr(), output.data_ptr(), batch_size,
...
void MaxPool2DLayer<T>::backward_impl(const Tensor<T> &gradient, Tensor<T> &grad_input,
  ...
  if (it_shape == micro_batch_input_shapes_.end()) {
    throw std::runtime_error("No cached input shape found for micro-batch ID in MaxPool2DLayer: " +
                             std::to_string(micro_batch_id));
  }
  ...
  backward_task_ = compute_max_pool_backward(gradient.data_ptr(), grad_input.data_ptr(), batch_size,
