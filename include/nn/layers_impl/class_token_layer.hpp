...
private:
  ...
  std::unique_ptr<Task> forward_task(const ConstTensor &input, Tensor &output,
                                     const ConstTensor &class_token, size_t batch_size,
                                     size_t seq_len, size_t embed_dim,
                                     const std::string &flow_id) const;

  template <typename IO_T, typename Param_T, typename Compute_T>
  std::unique_ptr<Task> backward_task(const ConstTensor &gradient, Tensor &grad_input,
                                      Tensor &class_token_gradients, const ConstTensor &class_token,
