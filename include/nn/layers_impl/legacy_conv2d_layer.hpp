...
private:
  ...
  std::unique_ptr<Task> compute_conv_forward_impl(
  ...
  template <typename IO_T, typename Param_T, typename Compute_T>
  std::unique_ptr<Task> cudnn_backward_bias(const ConstTensor &gradient, const Tensor &bias_grad,
