...
protected:
  void SetUp() override {
    ...
    small_tensor = Tensor<float>({1, 1, 2, 2}); // batch=1, channels=1, height=2, width=2
    small_tensor.fill(1.0f);

    large_tensor = Tensor<float>({2, 3, 4, 4}); // batch=2, channels=3, height=4, width=4
...
TEST_F(TensorTest, TensorAddition) {
  Tensor<float> tensor1({1, 1, 2, 2});
  Tensor<float> tensor2({1, 1, 2, 2});
...
TEST_F(TensorTest, DataAlignment) {
  Tensor<float> tensor({10, 10, 10, 10});
...
TEST_F(TensorTest, MultiBatchAccess) {
  Tensor<float> tensor({2, 1, 2, 2}); // 2 batches
...
TEST_F(TensorTest, MultiChannelAccess) {
  Tensor<float> tensor({1, 3, 2, 2}); // 3 channels
