...
TEST_F(ResidualBlockTest, IdentityShortcutForward) {
  ...
  residual.set_device(getCPU());
  residual.init();

  Tensor input = Tensor::create<float>({1, 1, 2, 2}, getCPU());
...
TEST_F(ResidualBlockTest, IdentityShortcutMultiChannel) {
  ...
  Tensor input = Tensor::create<float>({1, 2, 2, 2}, getCPU());
  float *input_data = input->data_as<float>();
  for (int i = 0; i < 8; ++i) {
    input_data[i] = 2.0f;
  }
...
TEST_F(ResidualBlockTest, IdentityShortcutMultiBatch) {
  ...
  Tensor input = Tensor::create<float>({2, 1, 2, 2}, getCPU());
  float *input_data = input->data_as<float>();
  for (int i = 0; i < 8; ++i) {
