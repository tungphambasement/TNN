...
exec torchrun --nnodes=2 --nproc_per_node=1 --node_rank=0 --master_addr="${MASTER_ADDR}" --master_port="${MASTER_PORT}" torch/torch_pipeline_parallel_async_mb.py --model "${MODEL}" --micro-bs "${MICRO_BS}" --seq-len "${SEQ_LEN}"
